resource "azurerm_application_gateway" "application_gateway" {
  ...
  dynamic "backend_address_pool" {
    ...
    content {
        ...
        ip_addresses = length(backend_address_pool.value.ip_addresses) > 0 ? backend_address_pool.value.ip_addresses : (length(local.agw_backend_ips) > 0 ? local.agw_backend_ips[each.key][backend_address_pool.key] : null)
    }
  }
